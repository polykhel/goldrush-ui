...
export class BookingFormComponent implements OnInit, CanComponentDeactivate {
  ...
  saveBooking(): void {
    ...
    this.bookingService
      .update(this.bookingId, updatedBooking)
      .pipe(finalize(() => (this.loading = false)))
      .subscribe({
  ...
  addNewPriceBreakdownRow(): void {
    ...
    if (this.editingPriceBreakdownIndex !== null) {
      this.toastService.warn(
        'Warning',
        'Please save or cancel your current edits first',
      );
      return;
    }
  ...
  savePriceBreakdown(): void {
    ...
    const serviceFeePct = usePercentage
      ? form.get('serviceFeePct')?.value || 0
      : 0;
    const serviceFeePerPax = usePercentage
    ...
    this.toastService.info(
      'Info',
      'Price breakdown saved. Remember to save your changes.',
    );
  }
  ...
  deletePriceBreakdown(index: number): void {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.toastService.info(
          'Info',
          'Price breakdown deleted. Remember to save your changes.',
        );
      },
    });
  }
  ...
  resetPaymentForm(): void {
    ...
    this.paymentForm.patchValue({
      ...
      amount: 0,
      remarks: this.calculateDefaultPaymentDescription(),
    });
  }
  ...
  openEditPaymentDialog(payment: PaymentHistory, index: number): void {
    ...
    if (payment.id) {
      this.toastService.warn(
        'Cannot Edit',
        'Payments already saved to the server cannot be edited',
      );
      return;
    }
    ...
    this.paymentForm.patchValue({
      ...
      feePercentage: payment.feePercentage || 3.5,
      remarks: payment.remarks || this.calculateDefaultPaymentDescription(),
    });
  }
  ...
  deletePayment(index: number): void {
    ...
    if (payment.id) {
      this.toastService.warn(
        'Cannot Delete',
        'Payments already saved to the server cannot be deleted',
      );
      return;
    }
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.toastService.info(
          'Info',
          'Payment deleted. Remember to save your changes.',
        );
      },
    });
  }
  ...
  addPayment(): void {
    ...
    const paymentMethod = formValue.paymentMethod || 'CASH';
    const feePercentage = formValue.feePercentage || 3.5;
    const remainingAmount = this.calculateRemainingAmount();

    // Calculate final amount including credit card fee if applicable
    let finalAmount = amount;
    let feeAmount = formValue.feeAmount || 0;
    ...
    const paymentType =
      finalAmount >= remainingAmount ? 'FULL_PAYMENT' : 'PARTIAL_PAYMENT';
    ...
    this.toastService.info(
      'Info',
      `Payment ${this.isEditingPayment ? 'updated' : 'added'}. Remember to save your changes.`,
    );
  }
  ...
  generateStatementOfAccount(format: 'pdf' | 'rtf' = 'pdf'): void {
    ...
    if (this.hasUnsavedChanges) {
      this.toastService.warn(
        'Unsaved Changes',
        'You have unsaved changes. Please save first before generating the statement of account',
      );
    }
  ...
  generatePaymentAcknowledgement(paymentHistoryId: string): void {
    ...
    this.loading = true;
    this.bookingService
      .generatePaymentAcknowledgement(this.bookingId, paymentHistoryId)
      .pipe(finalize(() => (this.loading = false)))
      .subscribe({
        next: (response) => {
          if (
        ...
        error: (error) => {
          this.toastService.error(
            'Error',
            'Failed to generate Payment Acknowledgement',
          );
  ...
  calculateTotals(): void {
    ...
    const totalServiceFees = this.calculateTotalServiceFees();
  ...
  getPaymentMethodLabel(value: string): string {
    return (
      this.paymentMethods.find((paymentMethod) => paymentMethod.value == value)
        ?.label ?? ''
    );
  }
  ...
  getPaymentTypeLabel(value: string): string {
    return (
      this.paymentTypes.find((paymentType) => paymentType.value == value)
        ?.label ?? ''
    );
  }

  private calculateDefaultPaymentDescription(): string {
    if (!this.booking) return '';

    const startDate = dayjs(this.booking.travelStartDate);
    const endDate = dayjs(this.booking.travelEndDate);
    const destination = this.booking.destination;

    // Calculate the difference in days
    const diffDays = endDate.diff(startDate, 'day') + 1; // +1 to include both start and end dates
    const nights = diffDays - 1; // Nights are one less than days

    return `${diffDays}D${nights}N ${destination}`;
  }
  ...
  private buildForm() {
    return this.fb.group({
      ...
      inquiryId: this.fb.control<string>(
        { value: '', disabled: true },
        Validators.required,
      ),
      ...
      travelStartDate: this.fb.control<Date>(
        { value: new Date(), disabled: true },
        Validators.required,
      ),
      travelEndDate: this.fb.control<Date>(
        { value: new Date(), disabled: true },
        Validators.required,
      ),
      ...
      packageType: this.fb.control<string>(
        { value: '', disabled: true },
        Validators.required,
      ),
      ...
      totalAmount: this.fb.control<number>({ value: 0, disabled: true }, [
        Validators.required,
        Validators.min(0),
      ]),
      paidAmount: this.fb.control<number>({ value: 0, disabled: true }, [
        Validators.required,
        Validators.min(0),
      ]),
      remainingAmount: this.fb.control<number>({ value: 0, disabled: true }, [
        Validators.required,
        Validators.min(0),
      ]),
      totalServiceFees: this.fb.control<number>({ value: 0, disabled: true }, [
        Validators.required,
        Validators.min(0),
      ]),
  ...
  private buildPaymentForm() {
    return this.fb.group({
      ...
      baseAmount: this.fb.control<number>(0),
      paymentMethod: this.fb.control<string>(
        this.bookingForm.get('modeOfPayment')?.value ?? 'CASH',
        Validators.required,
      ),
      feeAmount: this.fb.control<number>(0),
      feePercentage: this.fb.control<number>(3.5, [
        Validators.min(0),
        Validators.max(100),
      ]),
      remarks: this.fb.control<string>(''),
  ...
  private buildPriceBreakdownForm() {
    const form = this.fb.group({
      ...
      serviceFeePct: this.fb.control<number>(0, [
        Validators.required,
        Validators.min(0),
        Validators.max(100),
      ]),
      serviceFeePerPax: this.fb.control<number>(0, [
        Validators.required,
        Validators.min(0),
      ]),
    ...
    form.patchValue(
      {
        total,
        serviceFee,
        serviceFeePerPax: usePercentage
          ? updatedServiceFeePerPax
          : serviceFeePerPax,
        netAmount,
      },
      { emitEvent: false },
